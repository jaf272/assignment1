...
public final class IntrusionChainFinder {
  ...
  public static List<List<Hop>> findChains(
    ...
    // do the search
    addToSolutions(start, exploitsSorted, privMap, inventory, limitedCount, usedOncePerSys, visited, path, solutions, target, maxHops);
    // sort solutions
    ...
    // need to implement all my helper methods
  ...
  private static void addToSolutions(SystemInfo current, List<Exploit> exploitsSorted, Map<SystemInfo, Priv> privMap,
  ...
  Set<SystemInfo> visited, List<Hop> path, List<List<Hop>> solutions, SystemInfo target, int maxHops){
    ...
    for(Exploit ex: exploitsSorted){
      ...
      if(isLocal == true){
        ...
        boolean conds_hold = AllpreconditionsHold(ex, current, current, inventory, privMap, limitedCount, usedOncePerSys);
        if(!conds_hold){
        ...
        // recurse
        addToSolutions(current, exploitsSorted, privMap, inventory, limitedCount, usedOncePerSys, visited, path,
      ...
      if(current.routes != null){
        for(Route r: current.routes){
          ...
          boolean preconds_hold = AllpreconditionsHold(ex, current, dst, inventory, privMap, limitedCount, usedOncePerSys);
          if(!preconds_hold){
            continue;
          }
          ...
          visited.add(dst);
          addToSolutions(dst, exploitsSorted, privMap, inventory, limitedCount, usedOncePerSys, visited,
  ...
  // EXTRA UTILITIES THAT WILL BE HELPFUL
  ...
  private static int priviledgeOrder(Priv p){
    if(p == Priv.ADMIN){
      return 2;
    }
    else if(p == Priv.USER){
  ...
  private static boolean hasEnoughPriviledge(Priv have, Priv need){
    // no priviledge needed case
    if(need == null){
      return true;
    }
    // converting to numeric
    int have_numeric = priviledgeOrder(have);
    int need_numeric = priviledgeOrder(need);
  ...
  private static Priv priviledge_max(Priv a, Priv b){
    ...
    int a_number = priviledgeOrder(a);
    int b_number = priviledgeOrder(b);
  ...
  private static boolean AllpreconditionsHold(
    ...
    Map<String, Integer> globalUseCount, Map<String, Set<SystemInfo>> usedOncePerSystem){
      ...
      if(!hasEnoughPriviledge(have, ex.requiredPrivOnSource)){
        return false;
      }
